...
/**
 * Returns the Firestore singleton, or null if unavailable.
 * Used by api-call-logger and session-persistence — they treat null as "skip logging".
 */
...
export async function addCallToProfile(
  ...
  try {
    await getFirestore().collection(COLLECTION).doc(id).set({
