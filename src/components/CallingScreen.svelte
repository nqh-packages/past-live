...
<div
  class="fixed inset-0 z-50 flex flex-col items-center justify-center gap-6 bg-background"
  role="dialog"
...
>
  ...
  <div class="relative">
    <div
      class="w-24 h-24 rounded-full border-2 border-accent overflow-hidden bg-surface flex items-center justify-center"
    >
      {#if portrait}
        <img
          src={portrait}
  ...
  <div class="text-center space-y-1">
    <div
      class="font-mono text-sm font-semibold tracking-[0.16em] uppercase text-accent"
    >
      {characterName}
    </div>
    <div class="font-mono text-xs text-foreground/50 tracking-[0.08em]">
      {era}
    </div>
  </div>

  <!-- Status text: calling... → connected -->
  <div class="font-mono text-xs text-foreground/40 tracking-[0.12em]" aria-live="assertive">
    {#if showConnected}
      connected
    {:else}
