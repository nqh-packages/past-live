<script lang="ts">
  ...
  // ─── Read preview data from sessionStorage (survives full-page navigation) ─
  //
  // SessionPreview.svelte writes the entire PreviewData object to sessionStorage
  // before window.location.href navigation. Nano Stores do NOT survive that
  // navigation — previewData.get() returns null on /session mount.
  //
  // This is the authoritative source for voiceName, portrait, characterName,
  // historicalSetting, storyScript, and previewId used in connectSession().

  const previewRaw = typeof sessionStorage !== 'undefined'
    ? sessionStorage.getItem('past-live:preview')
    : null;

  let previewParsed: import('../stores/liveSession').PreviewData | null = null;
  try {
    previewParsed = previewRaw
      ? (JSON.parse(previewRaw) as import('../stores/liveSession').PreviewData)
      : null;
  } catch { /* malformed JSON — previewParsed stays null */ }

  const voiceName = previewParsed?.voiceName;
  const portrait = previewParsed?.avatar ?? previewParsed?.avatarUrl;

  // ─── Hydrate Nano Store from sessionStorage ────────────────────────────────
  //
  // Populates previewData for retry(), resolvedEra, and any future reader that
  // calls previewData.get() on /session — not just the initial connectSession().

  if (previewParsed && !previewData.get()) {
    previewData.set(previewParsed);
  }
  ...
  if (scenarioId || topic) {
    ...
    // Batch 5: pass storyScript so relay can pass it to buildSystemPrompt
    //
    // FIX (2026-03-16): previewData Nano Store is null after full-page navigation.
    // Use previewParsed (sessionStorage) as the authoritative source.
    // previewData.get() is checked first so hydration above works for retry() too.
    const previewCtx = previewData.get() ?? previewParsed;
    connectSession({
      scenarioId,
      topic,
      voiceName,
      ...
      storyScript: previewCtx?.storyScript,
      previewId: previewCtx?.previewId,
    });
  }
  ...
  function retry() {
    // Same FIX as initial connect: prefer hydrated store, fall back to sessionStorage
    const previewCtx = previewData.get() ?? previewParsed;
    connectSession({
      scenarioId,
      topic,
  ...
  // Human-readable status for screen reader announcements
  const statusAnnouncement = $derived.by(() => {
    switch ($status) {
      case 'connecting': return 'Connecting to the past...';
      case 'active': return `Connected. ${$characterName || 'Character'} is on the line.`;
      case 'reconnecting': return 'Reconnecting...';
      case 'ended': return 'Call ended.';
      case 'error': return `Connection error. ${$error || 'Please retry.'}`;
      default: return '';
    }
  });
</script>

<!-- Visually hidden live region: announces session status transitions to screen readers -->
...
{#if isWaitingForConnection}
  <div class="fixed inset-0 z-40 flex flex-col items-center justify-center bg-background/95" role="status" aria-label="Connecting to the past">
...
<!-- Error overlay -->
{#if $status === 'error'}
  <div class="fixed inset-0 z-40 flex flex-col items-center justify-center bg-background/95" role="alert" aria-live="assertive">
    <div class="max-w-xs text-center space-y-6">
      ...
      <div class="font-mono text-xs text-foreground/20">
      ...
      <div class="flex gap-4 justify-center">
        <button
          type="button"
          onclick={retry}
          class="border border-accent/30 text-accent font-mono text-sm tracking-[0.12em] uppercase px-5 min-h-11 py-2.5 rounded-sm hover:bg-accent/5 transition-colors"
        ...
        <a
          href="/app"
          class="inline-flex items-center border border-border text-foreground/40 font-mono text-sm tracking-[0.12em] uppercase px-5 min-h-11 py-2.5 rounded-sm hover:border-accent/20 hover:text-foreground/60 transition-colors"
          aria-label="Abort and return to home"
        >
          [ abort ]
        </a>
      </div>
...
{#if $status === 'active' && !$micEnabled}
  <div
    class="fixed bottom-28 left-1/2 -translate-x-1/2 z-30 font-mono text-xs text-foreground/30 tracking-[0.06em] pointer-events-none"
    aria-live="polite"
  >
