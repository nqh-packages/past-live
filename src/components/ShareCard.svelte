<script lang="ts">
  ...
  interface Props {
    /**
     * Bindable callback: parent can bind to this to trigger the download externally.
     * Example: <ShareCard bind:downloadFn={myRef} /> then call myRef?.()
     *
     * @why - SummaryActions action row lives outside ShareCard but needs to trigger
     *   the canvas download. Two-way bind avoids a global event bus.
     */
    downloadFn?: (() => void) | null;
  }

  let { downloadFn = $bindable(null) }: Props = $props();
  ...
  // ─── Mount: load summary and expose download fn to parent ────────────────

  onMount(() => {
    summary = loadSummaryArtifact();
    // Expose downloadCard to parent via bindable prop
    downloadFn = () => downloadCard();
  });
...
{#if summary}
  ...
  >
    <p
      class="font-mono text-xs text-accent tracking-[0.12em] uppercase mb-4"
      aria-hidden="true"
    >
      &gt; call receipt
    </p>
      ...
      <div class="absolute inset-0 flex flex-col px-5 pt-8 pb-5">
        ...
        <div class="flex-1 flex flex-col justify-center gap-4">
          ...
          <div>
            ...
            {#if scenarioTitle}
              <p
                class="font-mono text-2xs mt-1 leading-relaxed"
                style="color: {cardMuted};"
              >
                {scenarioTitle}
              </p>
          ...
          >
            <p
              class="font-serif text-xs italic leading-relaxed"
              style="color: {cardFg};"
            >
        ...
        <div>
          ...
          ></div>
          <p
            class="font-mono text-2xs text-right"
            style="color: {cardMuted};"
          >
            pastlive.site
          </p>
  ...
  <div class="flex justify-center mb-12">
    <button
      ...
      class="border font-mono text-sm tracking-[0.12em] uppercase px-6 py-3 rounded-sm
